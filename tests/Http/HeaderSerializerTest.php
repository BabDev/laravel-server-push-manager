...
final class HeaderSerializerTest extends TestCase
{
    ...
    public function testSerialize()
    {
        $links = [
            ...
            (new Link('alternate', '/5'))->withRel('next')->withAttribute('hreflang', 'de')->withAttribute('title', 'Hello'),
        ];

        $this->assertEquals(
            '</1>; rel="prerender",</2>; rel="dns-prefetch"; pr="0.7",</3>; rel="preload"; as="script",</4>; rel="preload"; as="image"; nopush,</5>; rel="alternate next"; hreflang="de"; title="Hello"',
            $this->serializer->serialize($links)
        );
    }
